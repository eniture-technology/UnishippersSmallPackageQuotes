...
class UnishippersSmpkgShipmentPackage
{
     ...
     * @return array
     */
    public function unishippersSmpkgOriginAddress($_product, $receiverZipCode)
    {
    ...
    {
        $packageArray = $this->dataHelper->unishippersSmallPlanInfo('ENUnishippersSmpkg');
        $planNumber = $packageArray['planNumber'];
        if (!empty($warehousList)) {
            $distance_array = [];
            if (count($warehousList) == 1) {
                $warehousList = reset($warehousList);
                return $this->unishippersSmpkgOriginArray($warehousList, $receiverZipCode, $planNumber);
            } elseif (count($warehousList) > 1 && ($planNumber == 0 || $planNumber == 1)) {
                return $this->unishippersSmpkgOriginArray($warehousList[0], $receiverZipCode, $planNumber);
            }

            $response  = $this->unishippersSmpkgAddress($warehousList);
            if (!empty($response)) {
                ...
                return $this->unishippersSmpkgOriginArray($originWithMinDist, $receiverZipCode, $planNumber);
            }
        }
    }
     ...
     * @return array
     */
    public function unishippersSmpkgOriginArray($shortOrigin, $receiverZipCode, $planNumber)
    {
     ...
     * @return array
     */
    public function unishippersSmpkgAddress($originAddress)
    {
        ...
        $curlRes = $this->dataHelper->unishippersSmSendCurlRequest(UnishippersSmConstants::GOOGLE_URL, $post);
